...
@Component({
   selector: 'app-carrerainstituto',
   templateUrl: './carrerainstituto.component.html',
   styleUrls: ['./carrerainstituto.component.scss']
})

export class CarreraInstitutoComponent implements OnInit {

   busy: Promise<any>;
   entidades: CarreraInstituto[];
   entidadSeleccionada: CarreraInstituto;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: CarreraInstitutoService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: CarreraInstituto): boolean {
      return true;
   }
   ...
   crearEntidad(): CarreraInstituto {
      const nuevoCarreraInstituto = new CarreraInstituto();
      nuevoCarreraInstituto.id = 0;
      return nuevoCarreraInstituto;
   }

   add(entidadNueva: CarreraInstituto): void {
   ...
   update(entidadParaActualizar: CarreraInstituto): void {
   ...
   delete(entidadParaBorrar: CarreraInstituto): void {
   ...
   refresh(): void {
      ...
      this.entidades = CarreraInstituto[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: CarreraInstituto): void {
