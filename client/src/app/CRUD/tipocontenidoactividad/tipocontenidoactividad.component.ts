...
@Component({
   selector: 'app-tipocontenidoactividad',
   templateUrl: './tipocontenidoactividad.component.html',
   styleUrls: ['./tipocontenidoactividad.component.scss']
})

export class TipoContenidoActividadComponent implements OnInit {

   busy: Promise<any>;
   entidades: TipoContenidoActividad[];
   entidadSeleccionada: TipoContenidoActividad;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: TipoContenidoActividadService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: TipoContenidoActividad): boolean {
      return true;
   }
   ...
   crearEntidad(): TipoContenidoActividad {
      const nuevoTipoContenidoActividad = new TipoContenidoActividad();
      nuevoTipoContenidoActividad.id = 0;
      return nuevoTipoContenidoActividad;
   }

   add(entidadNueva: TipoContenidoActividad): void {
   ...
   update(entidadParaActualizar: TipoContenidoActividad): void {
   ...
   delete(entidadParaBorrar: TipoContenidoActividad): void {
   ...
   refresh(): void {
      ...
      this.entidades = TipoContenidoActividad[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: TipoContenidoActividad): void {
