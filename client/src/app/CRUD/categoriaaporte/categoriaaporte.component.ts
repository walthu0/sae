...
@Component({
   selector: 'app-categoriaaporte',
   templateUrl: './categoriaaporte.component.html',
   styleUrls: ['./categoriaaporte.component.scss']
})

export class CategoriaAporteComponent implements OnInit {

   busy: Promise<any>;
   entidades: CategoriaAporte[];
   entidadSeleccionada: CategoriaAporte;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: CategoriaAporteService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: CategoriaAporte): boolean {
      return true;
   }
   ...
   crearEntidad(): CategoriaAporte {
      const nuevoCategoriaAporte = new CategoriaAporte();
      nuevoCategoriaAporte.id = 0;
      return nuevoCategoriaAporte;
   }

   add(entidadNueva: CategoriaAporte): void {
   ...
   update(entidadParaActualizar: CategoriaAporte): void {
   ...
   delete(entidadParaBorrar: CategoriaAporte): void {
   ...
   refresh(): void {
      ...
      this.entidades = CategoriaAporte[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: CategoriaAporte): void {
