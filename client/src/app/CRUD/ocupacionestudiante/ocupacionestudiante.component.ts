...
@Component({
   selector: 'app-ocupacionestudiante',
   templateUrl: './ocupacionestudiante.component.html',
   styleUrls: ['./ocupacionestudiante.component.scss']
})

export class OcupacionEstudianteComponent implements OnInit {

   busy: Promise<any>;
   entidades: OcupacionEstudiante[];
   entidadSeleccionada: OcupacionEstudiante;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: OcupacionEstudianteService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: OcupacionEstudiante): boolean {
      return true;
   }
   ...
   crearEntidad(): OcupacionEstudiante {
      const nuevoOcupacionEstudiante = new OcupacionEstudiante();
      nuevoOcupacionEstudiante.id = 0;
      return nuevoOcupacionEstudiante;
   }

   add(entidadNueva: OcupacionEstudiante): void {
   ...
   update(entidadParaActualizar: OcupacionEstudiante): void {
   ...
   delete(entidadParaBorrar: OcupacionEstudiante): void {
   ...
   refresh(): void {
      ...
      this.entidades = OcupacionEstudiante[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: OcupacionEstudiante): void {
