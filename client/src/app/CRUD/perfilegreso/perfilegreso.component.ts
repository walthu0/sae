...
@Component({
   selector: 'app-perfilegreso',
   templateUrl: './perfilegreso.component.html',
   styleUrls: ['./perfilegreso.component.scss']
})

export class PerfilEgresoComponent implements OnInit {

   busy: Promise<any>;
   entidades: PerfilEgreso[];
   entidadSeleccionada: PerfilEgreso;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: PerfilEgresoService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: PerfilEgreso): boolean {
      return true;
   }
   ...
   crearEntidad(): PerfilEgreso {
      const nuevoPerfilEgreso = new PerfilEgreso();
      nuevoPerfilEgreso.id = 0;
      return nuevoPerfilEgreso;
   }

   add(entidadNueva: PerfilEgreso): void {
   ...
   update(entidadParaActualizar: PerfilEgreso): void {
   ...
   delete(entidadParaBorrar: PerfilEgreso): void {
   ...
   onSelect(entidadActual: PerfilEgreso): void {
