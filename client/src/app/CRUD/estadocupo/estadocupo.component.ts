...
@Component({
   selector: 'app-estadocupo',
   templateUrl: './estadocupo.component.html',
   styleUrls: ['./estadocupo.component.scss']
})

export class EstadoCupoComponent implements OnInit {

   busy: Promise<any>;
   entidades: EstadoCupo[];
   entidadSeleccionada: EstadoCupo;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: EstadoCupoService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: EstadoCupo): boolean {
      return true;
   }
   ...
   crearEntidad(): EstadoCupo {
      const nuevoEstadoCupo = new EstadoCupo();
      nuevoEstadoCupo.id = 0;
      return nuevoEstadoCupo;
   }

   add(entidadNueva: EstadoCupo): void {
   ...
   update(entidadParaActualizar: EstadoCupo): void {
   ...
   delete(entidadParaBorrar: EstadoCupo): void {
   ...
   onSelect(entidadActual: EstadoCupo): void {
