...
@Component({
   selector: 'app-porcentajeaportefinal',
   templateUrl: './porcentajeaportefinal.component.html',
   styleUrls: ['./porcentajeaportefinal.component.scss']
})

export class PorcentajeAporteFinalComponent implements OnInit {

   busy: Promise<any>;
   entidades: PorcentajeAporteFinal[];
   entidadSeleccionada: PorcentajeAporteFinal;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: PorcentajeAporteFinalService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: PorcentajeAporteFinal): boolean {
      return true;
   }
   ...
   crearEntidad(): PorcentajeAporteFinal {
      const nuevoPorcentajeAporteFinal = new PorcentajeAporteFinal();
      nuevoPorcentajeAporteFinal.id = 0;
      return nuevoPorcentajeAporteFinal;
   }

   add(entidadNueva: PorcentajeAporteFinal): void {
   ...
   update(entidadParaActualizar: PorcentajeAporteFinal): void {
   ...
   delete(entidadParaBorrar: PorcentajeAporteFinal): void {
   ...
   refresh(): void {
      ...
      this.entidades = PorcentajeAporteFinal[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: PorcentajeAporteFinal): void {
