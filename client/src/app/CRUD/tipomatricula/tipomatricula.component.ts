...
@Component({
   selector: 'app-tipomatricula',
   templateUrl: './tipomatricula.component.html',
   styleUrls: ['./tipomatricula.component.scss']
})

export class TipoMatriculaComponent implements OnInit {

   busy: Promise<any>;
   entidades: TipoMatricula[];
   entidadSeleccionada: TipoMatricula;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: TipoMatriculaService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: TipoMatricula): boolean {
      return true;
   }
   ...
   crearEntidad(): TipoMatricula {
      const nuevoTipoMatricula = new TipoMatricula();
      nuevoTipoMatricula.id = 0;
      return nuevoTipoMatricula;
   }

   add(entidadNueva: TipoMatricula): void {
   ...
   update(entidadParaActualizar: TipoMatricula): void {
   ...
   delete(entidadParaBorrar: TipoMatricula): void {
   ...
   refresh(): void {
      ...
      this.entidades = TipoMatricula[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: TipoMatricula): void {
