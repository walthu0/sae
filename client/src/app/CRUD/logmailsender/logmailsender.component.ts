...
@Component({
   selector: 'app-logmailsender',
   templateUrl: './logmailsender.component.html',
   styleUrls: ['./logmailsender.component.scss']
})

export class LogMailSenderComponent implements OnInit {

   busy: Promise<any>;
   entidades: LogMailSender[];
   entidadSeleccionada: LogMailSender;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: LogMailSenderService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: LogMailSender): boolean {
      return true;
   }
   ...
   crearEntidad(): LogMailSender {
      const nuevoLogMailSender = new LogMailSender();
      nuevoLogMailSender.id = 0;
      return nuevoLogMailSender;
   }

   add(entidadNueva: LogMailSender): void {
   ...
   update(entidadParaActualizar: LogMailSender): void {
   ...
   delete(entidadParaBorrar: LogMailSender): void {
   ...
   refresh(): void {
      ...
      this.entidades = LogMailSender[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: LogMailSender): void {
