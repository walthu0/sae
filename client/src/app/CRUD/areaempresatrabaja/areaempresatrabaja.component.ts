...
@Component({
   selector: 'app-areaempresatrabaja',
   templateUrl: './areaempresatrabaja.component.html',
   styleUrls: ['./areaempresatrabaja.component.scss']
})

export class AreaEmpresaTrabajaComponent implements OnInit {

   busy: Promise<any>;
   entidades: AreaEmpresaTrabaja[];
   entidadSeleccionada: AreaEmpresaTrabaja;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: AreaEmpresaTrabajaService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: AreaEmpresaTrabaja): boolean {
      return true;
   }
   ...
   crearEntidad(): AreaEmpresaTrabaja {
      const nuevoAreaEmpresaTrabaja = new AreaEmpresaTrabaja();
      nuevoAreaEmpresaTrabaja.id = 0;
      return nuevoAreaEmpresaTrabaja;
   }

   add(entidadNueva: AreaEmpresaTrabaja): void {
   ...
   update(entidadParaActualizar: AreaEmpresaTrabaja): void {
   ...
   delete(entidadParaBorrar: AreaEmpresaTrabaja): void {
   ...
   refresh(): void {
      ...
      this.entidades = AreaEmpresaTrabaja[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: AreaEmpresaTrabaja): void {
