...
@Component({
   selector: 'app-plansemanaobjetivo',
   templateUrl: './plansemanaobjetivo.component.html',
   styleUrls: ['./plansemanaobjetivo.component.scss']
})

export class PlanSemanaObjetivoComponent implements OnInit {

   busy: Promise<any>;
   entidades: PlanSemanaObjetivo[];
   entidadSeleccionada: PlanSemanaObjetivo;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: PlanSemanaObjetivoService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: PlanSemanaObjetivo): boolean {
      return true;
   }
   ...
   crearEntidad(): PlanSemanaObjetivo {
      const nuevoPlanSemanaObjetivo = new PlanSemanaObjetivo();
      nuevoPlanSemanaObjetivo.id = 0;
      return nuevoPlanSemanaObjetivo;
   }

   add(entidadNueva: PlanSemanaObjetivo): void {
   ...
   update(entidadParaActualizar: PlanSemanaObjetivo): void {
   ...
   delete(entidadParaBorrar: PlanSemanaObjetivo): void {
   ...
   refresh(): void {
      ...
      this.entidades = PlanSemanaObjetivo[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: PlanSemanaObjetivo): void {
