...
@Component({
   selector: 'app-cargocarrera',
   templateUrl: './cargocarrera.component.html',
   styleUrls: ['./cargocarrera.component.scss']
})

export class CargoCarreraComponent implements OnInit {

   busy: Promise<any>;
   entidades: CargoCarrera[];
   entidadSeleccionada: CargoCarrera;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: CargoCarreraService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: CargoCarrera): boolean {
      return true;
   }
   ...
   crearEntidad(): CargoCarrera {
      const nuevoCargoCarrera = new CargoCarrera();
      nuevoCargoCarrera.id = 0;
      return nuevoCargoCarrera;
   }

   add(entidadNueva: CargoCarrera): void {
   ...
   update(entidadParaActualizar: CargoCarrera): void {
   ...
   delete(entidadParaBorrar: CargoCarrera): void {
   ...
   onSelect(entidadActual: CargoCarrera): void {
