...
@Component({
   selector: 'app-aportefinal',
   templateUrl: './aportefinal.component.html',
   styleUrls: ['./aportefinal.component.scss']
})

export class AporteFinalComponent implements OnInit {

   busy: Promise<any>;
   entidades: AporteFinal[];
   entidadSeleccionada: AporteFinal;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: AporteFinalService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: AporteFinal): boolean {
      return true;
   }
   ...
   crearEntidad(): AporteFinal {
      const nuevoAporteFinal = new AporteFinal();
      nuevoAporteFinal.id = 0;
      return nuevoAporteFinal;
   }

   add(entidadNueva: AporteFinal): void {
   ...
   update(entidadParaActualizar: AporteFinal): void {
   ...
   delete(entidadParaBorrar: AporteFinal): void {
   ...
   onSelect(entidadActual: AporteFinal): void {
