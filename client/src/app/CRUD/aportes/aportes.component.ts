...
export class AportesComponent implements OnInit {
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: AportesService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: Aportes): boolean {
      return true;
   }
   ...
   crearEntidad(): Aportes {
      const nuevoAportes = new Aportes();
      nuevoAportes.id = 0;
      return nuevoAportes;
   }

   add(entidadNueva: Aportes): void {
   ...
   update(entidadParaActualizar: Aportes): void {
   ...
   delete(entidadParaBorrar: Aportes): void {
   ...
   onSelect(entidadActual: Aportes): void {
