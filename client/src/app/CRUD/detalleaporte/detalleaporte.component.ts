...
@Component({
   selector: 'app-detalleaporte',
   templateUrl: './detalleaporte.component.html',
   styleUrls: ['./detalleaporte.component.scss']
})

export class DetalleAporteComponent implements OnInit {

   busy: Promise<any>;
   entidades: DetalleAporte[];
   entidadSeleccionada: DetalleAporte;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: DetalleAporteService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: DetalleAporte): boolean {
      return true;
   }
   ...
   crearEntidad(): DetalleAporte {
      const nuevoDetalleAporte = new DetalleAporte();
      nuevoDetalleAporte.id = 0;
      return nuevoDetalleAporte;
   }

   add(entidadNueva: DetalleAporte): void {
   ...
   update(entidadParaActualizar: DetalleAporte): void {
   ...
   delete(entidadParaBorrar: DetalleAporte): void {
   ...
   refresh(): void {
      ...
      this.entidades = DetalleAporte[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: DetalleAporte): void {
