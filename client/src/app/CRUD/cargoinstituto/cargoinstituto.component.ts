...
@Component({
   selector: 'app-cargoinstituto',
   templateUrl: './cargoinstituto.component.html',
   styleUrls: ['./cargoinstituto.component.scss']
})

export class CargoInstitutoComponent implements OnInit {

   busy: Promise<any>;
   entidades: CargoInstituto[];
   entidadSeleccionada: CargoInstituto;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: CargoInstitutoService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: CargoInstituto): boolean {
      return true;
   }
   ...
   crearEntidad(): CargoInstituto {
      const nuevoCargoInstituto = new CargoInstituto();
      nuevoCargoInstituto.id = 0;
      return nuevoCargoInstituto;
   }

   add(entidadNueva: CargoInstituto): void {
   ...
   update(entidadParaActualizar: CargoInstituto): void {
   ...
   delete(entidadParaBorrar: CargoInstituto): void {
   ...
   refresh(): void {
      ...
      this.entidades = CargoInstituto[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: CargoInstituto): void {
