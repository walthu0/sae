...
@Component({
   selector: 'app-asignaturacorrequisito',
   templateUrl: './asignaturacorrequisito.component.html',
   styleUrls: ['./asignaturacorrequisito.component.scss']
})

export class AsignaturaCorrequisitoComponent implements OnInit {

   busy: Promise<any>;
   entidades: AsignaturaCorrequisito[];
   entidadSeleccionada: AsignaturaCorrequisito;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: AsignaturaCorrequisitoService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: AsignaturaCorrequisito): boolean {
      return true;
   }
   ...
   crearEntidad(): AsignaturaCorrequisito {
      const nuevoAsignaturaCorrequisito = new AsignaturaCorrequisito();
      nuevoAsignaturaCorrequisito.id = 0;
      return nuevoAsignaturaCorrequisito;
   }

   add(entidadNueva: AsignaturaCorrequisito): void {
   ...
   update(entidadParaActualizar: AsignaturaCorrequisito): void {
   ...
   delete(entidadParaBorrar: AsignaturaCorrequisito): void {
   ...
   refresh(): void {
      ...
      this.entidades = AsignaturaCorrequisito[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: AsignaturaCorrequisito): void {
