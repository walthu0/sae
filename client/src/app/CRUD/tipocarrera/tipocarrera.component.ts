...
@Component({
   selector: 'app-tipocarrera',
   templateUrl: './tipocarrera.component.html',
   styleUrls: ['./tipocarrera.component.scss']
})

export class TipoCarreraComponent implements OnInit {

   busy: Promise<any>;
   entidades: TipoCarrera[];
   entidadSeleccionada: TipoCarrera;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: TipoCarreraService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: TipoCarrera): boolean {
      return true;
   }
   ...
   crearEntidad(): TipoCarrera {
      const nuevoTipoCarrera = new TipoCarrera();
      nuevoTipoCarrera.id = 0;
      return nuevoTipoCarrera;
   }

   add(entidadNueva: TipoCarrera): void {
   ...
   update(entidadParaActualizar: TipoCarrera): void {
   ...
   delete(entidadParaBorrar: TipoCarrera): void {
   ...
   onSelect(entidadActual: TipoCarrera): void {
