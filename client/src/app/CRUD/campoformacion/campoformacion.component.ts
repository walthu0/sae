...
@Component({
   selector: 'app-campoformacion',
   templateUrl: './campoformacion.component.html',
   styleUrls: ['./campoformacion.component.scss']
})

export class CampoFormacionComponent implements OnInit {

   busy: Promise<any>;
   entidades: CampoFormacion[];
   entidadSeleccionada: CampoFormacion;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: CampoFormacionService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: CampoFormacion): boolean {
      return true;
   }
   ...
   crearEntidad(): CampoFormacion {
      const nuevoCampoFormacion = new CampoFormacion();
      nuevoCampoFormacion.id = 0;
      return nuevoCampoFormacion;
   }

   add(entidadNueva: CampoFormacion): void {
   ...
   update(entidadParaActualizar: CampoFormacion): void {
   ...
   delete(entidadParaBorrar: CampoFormacion): void {
   ...
   refresh(): void {
      ...
      this.entidades = CampoFormacion[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: CampoFormacion): void {
