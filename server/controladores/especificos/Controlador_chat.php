...
class Controlador_chat extends Controlador_Base
{
    function consultar_salas($args)
    {
        ...
        if ($rol === "DOCENTE") {
            ...
            array_push($salas, ["nombre" => "YAVIRAC", "miembros" => $this->ejecutar_sql($sql, $parametros)]);
            $sql = "SELECT DISTINCT idPersona, CONCAT(Persona.nombre1, ' ', Persona.nombre2, ' ', Persona.apellido1, ' ', Persona.apellido2) as nombreCompleto FROM Docente INNER JOIN Persona ON Persona.id = Estudiante.idPersona;";
            array_push($salas, ["nombre" => "DOCENTES", "miembros" => $this->ejecutar_sql($sql, $parametros)]);
            ...
            foreach ($Asignaturas as $asignatura) {
                ...
                array_push($salas, ["nombre" => $asignatura['nombre'], "miembros" => $this->ejecutar_sql($sql, $parametros)]);
            }
            ...
            foreach ($Carreras as $carrera) {
                ...
                array_push($salas, ["nombre" => $carrera['nombre'], "miembros" => $this->ejecutar_sql($sql, $parametros)]);
            }
        }
        if ($rol === "ESTUDIANTE") {
            ...
            array_push($salas, ["nombre" => "ESTUDIANTES", "miembros" => $this->ejecutar_sql($sql, $parametros)]);
