...
class Controlador_cargo extends Controlador_Base
{
   function crear($args)
   {
      $cargo = new Cargo($args["id"],$args["descripcion"]);
      $sql = "INSERT INTO Cargo (descripcion) VALUES (?);";
      $parametros = array($cargo->descripcion);
   ...
   {
      $cargo = new Cargo($args["id"],$args["descripcion"]);
      $parametros = array($cargo->descripcion,$cargo->id);
      $sql = "UPDATE Cargo SET descripcion = ? WHERE id = ?;";
      $respuesta = $this->conexion->ejecutarConsulta($sql,$parametros);
      if(is_null($respuesta[0])){
         return true;
      }else{
