...
@Component({
   selector: 'app-plansemana',
   templateUrl: './plansemana.component.html',
   styleUrls: ['./plansemana.component.scss']
})

export class PlanSemanaComponent implements OnInit {

   busy: Promise<any>;
   entidades: PlanSemana[];
   entidadSeleccionada: PlanSemana;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: PlanSemanaService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: PlanSemana): boolean {
      return true;
   }
   ...
   crearEntidad(): PlanSemana {
      const nuevoPlanSemana = new PlanSemana();
      nuevoPlanSemana.id = 0;
      return nuevoPlanSemana;
   }

   add(entidadNueva: PlanSemana): void {
   ...
   update(entidadParaActualizar: PlanSemana): void {
   ...
   delete(entidadParaBorrar: PlanSemana): void {
   ...
   onSelect(entidadActual: PlanSemana): void {
