...
@Component({
   selector: 'app-plansemanametodologia',
   templateUrl: './plansemanametodologia.component.html',
   styleUrls: ['./plansemanametodologia.component.scss']
})

export class PlanSemanaMetodologiaComponent implements OnInit {

   busy: Promise<any>;
   entidades: PlanSemanaMetodologia[];
   entidadSeleccionada: PlanSemanaMetodologia;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: PlanSemanaMetodologiaService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: PlanSemanaMetodologia): boolean {
      return true;
   }
   ...
   crearEntidad(): PlanSemanaMetodologia {
      const nuevoPlanSemanaMetodologia = new PlanSemanaMetodologia();
      nuevoPlanSemanaMetodologia.id = 0;
      return nuevoPlanSemanaMetodologia;
   }

   add(entidadNueva: PlanSemanaMetodologia): void {
   ...
   update(entidadParaActualizar: PlanSemanaMetodologia): void {
   ...
   delete(entidadParaBorrar: PlanSemanaMetodologia): void {
   ...
   refresh(): void {
      ...
      this.entidades = PlanSemanaMetodologia[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: PlanSemanaMetodologia): void {
