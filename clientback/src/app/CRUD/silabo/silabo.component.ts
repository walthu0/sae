...
export class SilaboComponent implements OnInit {
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: SilaboService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: Silabo): boolean {
      return true;
   }
   ...
   crearEntidad(): Silabo {
      const nuevoSilabo = new Silabo();
      nuevoSilabo.id = 0;
      return nuevoSilabo;
   }

   add(entidadNueva: Silabo): void {
   ...
   update(entidadParaActualizar: Silabo): void {
   ...
   delete(entidadParaBorrar: Silabo): void {
   ...
   onSelect(entidadActual: Silabo): void {
