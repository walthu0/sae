...
@Component({
   selector: 'app-tipobachillerato',
   templateUrl: './tipobachillerato.component.html',
   styleUrls: ['./tipobachillerato.component.scss']
})

export class TipoBachilleratoComponent implements OnInit {

   busy: Promise<any>;
   entidades: TipoBachillerato[];
   entidadSeleccionada: TipoBachillerato;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: TipoBachilleratoService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: TipoBachillerato): boolean {
      return true;
   }
   ...
   crearEntidad(): TipoBachillerato {
      const nuevoTipoBachillerato = new TipoBachillerato();
      nuevoTipoBachillerato.id = 0;
      return nuevoTipoBachillerato;
   }

   add(entidadNueva: TipoBachillerato): void {
   ...
   update(entidadParaActualizar: TipoBachillerato): void {
   ...
   delete(entidadParaBorrar: TipoBachillerato): void {
   ...
   refresh(): void {
      ...
      this.entidades = TipoBachillerato[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: TipoBachillerato): void {
