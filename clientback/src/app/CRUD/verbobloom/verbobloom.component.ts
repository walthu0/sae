...
@Component({
   selector: 'app-verbobloom',
   templateUrl: './verbobloom.component.html',
   styleUrls: ['./verbobloom.component.scss']
})

export class VerboBloomComponent implements OnInit {

   busy: Promise<any>;
   entidades: VerboBloom[];
   entidadSeleccionada: VerboBloom;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: VerboBloomService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: VerboBloom): boolean {
      return true;
   }
   ...
   crearEntidad(): VerboBloom {
      const nuevoVerboBloom = new VerboBloom();
      nuevoVerboBloom.id = 0;
      return nuevoVerboBloom;
   }

   add(entidadNueva: VerboBloom): void {
   ...
   update(entidadParaActualizar: VerboBloom): void {
   ...
   delete(entidadParaBorrar: VerboBloom): void {
   ...
   onSelect(entidadActual: VerboBloom): void {
