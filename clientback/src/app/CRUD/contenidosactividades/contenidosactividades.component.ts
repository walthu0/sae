...
@Component({
   selector: 'app-contenidosactividades',
   templateUrl: './contenidosactividades.component.html',
   styleUrls: ['./contenidosactividades.component.scss']
})

export class ContenidosActividadesComponent implements OnInit {

   busy: Promise<any>;
   entidades: ContenidosActividades[];
   entidadSeleccionada: ContenidosActividades;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: ContenidosActividadesService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: ContenidosActividades): boolean {
      return true;
   }
   ...
   crearEntidad(): ContenidosActividades {
      const nuevoContenidosActividades = new ContenidosActividades();
      nuevoContenidosActividades.id = 0;
      return nuevoContenidosActividades;
   }

   add(entidadNueva: ContenidosActividades): void {
   ...
   update(entidadParaActualizar: ContenidosActividades): void {
   ...
   delete(entidadParaBorrar: ContenidosActividades): void {
   ...
   refresh(): void {
      ...
      this.entidades = ContenidosActividades[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: ContenidosActividades): void {
