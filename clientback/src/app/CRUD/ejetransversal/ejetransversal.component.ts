...
@Component({
   selector: 'app-ejetransversal',
   templateUrl: './ejetransversal.component.html',
   styleUrls: ['./ejetransversal.component.scss']
})

export class EjeTransversalComponent implements OnInit {

   busy: Promise<any>;
   entidades: EjeTransversal[];
   entidadSeleccionada: EjeTransversal;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: EjeTransversalService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: EjeTransversal): boolean {
      return true;
   }
   ...
   crearEntidad(): EjeTransversal {
      const nuevoEjeTransversal = new EjeTransversal();
      nuevoEjeTransversal.id = 0;
      return nuevoEjeTransversal;
   }

   add(entidadNueva: EjeTransversal): void {
   ...
   update(entidadParaActualizar: EjeTransversal): void {
   ...
   delete(entidadParaBorrar: EjeTransversal): void {
   ...
   refresh(): void {
      ...
      this.entidades = EjeTransversal[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: EjeTransversal): void {
