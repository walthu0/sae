...
@Component({
   selector: 'app-tecnicaevaluacion',
   templateUrl: './tecnicaevaluacion.component.html',
   styleUrls: ['./tecnicaevaluacion.component.scss']
})

export class TecnicaEvaluacionComponent implements OnInit {

   busy: Promise<any>;
   entidades: TecnicaEvaluacion[];
   entidadSeleccionada: TecnicaEvaluacion;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: TecnicaEvaluacionService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: TecnicaEvaluacion): boolean {
      return true;
   }
   ...
   crearEntidad(): TecnicaEvaluacion {
      const nuevoTecnicaEvaluacion = new TecnicaEvaluacion();
      nuevoTecnicaEvaluacion.id = 0;
      return nuevoTecnicaEvaluacion;
   }

   add(entidadNueva: TecnicaEvaluacion): void {
   ...
   update(entidadParaActualizar: TecnicaEvaluacion): void {
   ...
   delete(entidadParaBorrar: TecnicaEvaluacion): void {
   ...
   refresh(): void {
      ...
      this.entidades = TecnicaEvaluacion[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: TecnicaEvaluacion): void {
