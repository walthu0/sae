...
@Component({
   selector: 'app-silaboevidenciasra',
   templateUrl: './silaboevidenciasra.component.html',
   styleUrls: ['./silaboevidenciasra.component.scss']
})

export class SilaboEvidenciasRaComponent implements OnInit {

   busy: Promise<any>;
   entidades: SilaboEvidenciasRa[];
   entidadSeleccionada: SilaboEvidenciasRa;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: SilaboEvidenciasRaService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: SilaboEvidenciasRa): boolean {
      return true;
   }
   ...
   crearEntidad(): SilaboEvidenciasRa {
      const nuevoSilaboEvidenciasRa = new SilaboEvidenciasRa();
      nuevoSilaboEvidenciasRa.id = 0;
      return nuevoSilaboEvidenciasRa;
   }

   add(entidadNueva: SilaboEvidenciasRa): void {
   ...
   update(entidadParaActualizar: SilaboEvidenciasRa): void {
   ...
   delete(entidadParaBorrar: SilaboEvidenciasRa): void {
   ...
   refresh(): void {
      ...
      this.entidades = SilaboEvidenciasRa[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: SilaboEvidenciasRa): void {
