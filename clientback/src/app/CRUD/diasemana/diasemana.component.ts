...
@Component({
   ...
   styleUrls: ['./diasemana.component.scss']
})
...
export class DiaSemanaComponent implements OnInit {
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: DiaSemanaService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: DiaSemana): boolean {
      return true;
   }
   ...
   crearEntidad(): DiaSemana {
      const nuevoDiaSemana = new DiaSemana();
      nuevoDiaSemana.id = 0;
      return nuevoDiaSemana;
   }

   add(entidadNueva: DiaSemana): void {
   ...
   update(entidadParaActualizar: DiaSemana): void {
   ...
   delete(entidadParaBorrar: DiaSemana): void {
   ...
   onSelect(entidadActual: DiaSemana): void {
