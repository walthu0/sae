...
@Component({
   selector: 'app-categoriamigratoria',
   templateUrl: './categoriamigratoria.component.html',
   styleUrls: ['./categoriamigratoria.component.scss']
})

export class CategoriaMigratoriaComponent implements OnInit {

   busy: Promise<any>;
   entidades: CategoriaMigratoria[];
   entidadSeleccionada: CategoriaMigratoria;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: CategoriaMigratoriaService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: CategoriaMigratoria): boolean {
      return true;
   }
   ...
   crearEntidad(): CategoriaMigratoria {
      const nuevoCategoriaMigratoria = new CategoriaMigratoria();
      nuevoCategoriaMigratoria.id = 0;
      return nuevoCategoriaMigratoria;
   }

   add(entidadNueva: CategoriaMigratoria): void {
   ...
   update(entidadParaActualizar: CategoriaMigratoria): void {
   ...
   delete(entidadParaBorrar: CategoriaMigratoria): void {
   ...
   refresh(): void {
      ...
      this.entidades = CategoriaMigratoria[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: CategoriaMigratoria): void {
