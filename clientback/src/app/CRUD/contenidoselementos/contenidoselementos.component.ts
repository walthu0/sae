...
@Component({
   selector: 'app-contenidoselementos',
   templateUrl: './contenidoselementos.component.html',
   styleUrls: ['./contenidoselementos.component.scss']
})

export class ContenidosElementosComponent implements OnInit {

   busy: Promise<any>;
   entidades: ContenidosElementos[];
   entidadSeleccionada: ContenidosElementos;
   pagina: 1;
   tamanoPagina: 20;
   paginaActual: number;
   paginaUltima: number;
   ...
   constructor(public toastr: ToastsManager, vcr: ViewContainerRef, private dataService: ContenidosElementosService, private modalService: NgbModal) {
   ...
   isValid(entidadPorEvaluar: ContenidosElementos): boolean {
      return true;
   }
   ...
   crearEntidad(): ContenidosElementos {
      const nuevoContenidosElementos = new ContenidosElementos();
      nuevoContenidosElementos.id = 0;
      return nuevoContenidosElementos;
   }

   add(entidadNueva: ContenidosElementos): void {
   ...
   update(entidadParaActualizar: ContenidosElementos): void {
   ...
   delete(entidadParaBorrar: ContenidosElementos): void {
   ...
   refresh(): void {
      ...
      this.entidades = ContenidosElementos[0];
      this.entidadSeleccionada = this.crearEntidad();
   }
   ...
   onSelect(entidadActual: ContenidosElementos): void {
